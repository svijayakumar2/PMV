...
def reset_prover_for_round(config):
    """Reset only the prover to base checkpoint (Stackelberg follower)"""
    
    print("GPU cleanup for prover")
    ...
    print(f"Reset prover, GPU memory: {torch.cuda.memory_allocated()/1e9:.2f} GB")
    return prover


def initialize_verifiers_once(config):
    """Initialize verifiers once at the start (Stackelberg leaders)"""
    ...
    torch.cuda.empty_cache()
    print(f"Initialized {len(verifiers)} verifiers (will persist across rounds)")
    return verifiers
